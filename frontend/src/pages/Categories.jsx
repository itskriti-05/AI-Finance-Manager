...
export default function Categories() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      setError("We couldn't load your category data. Please try again.");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const discretionarySpending = useMemo(() => {
    return categoryBudgets
      ...
      .reduce((total, item) => total + Number(item.last30DaySpend || 0), 0);
  }, [categoryBudgets]);
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-border bg-card p-10 text-center">
          ...
          <p className="mx-auto mt-2 max-w-sm text-sm leading-relaxed text-muted-foreground">
            Upload a bank statement and Finwise will organize your spending into
        ...
        <>
          ...
          <div className="mt-6 rounded-2xl border border-border bg-card p-5">
            <div className="flex items-start justify-between">
              <div>
                <h2 className="text-sm font-semibold">Spending distribution</h2>
            ...
            <div className="mt-6 grid grid-cols-1 gap-8 lg:grid-cols-[260px_1fr]">
              ...
              <div className="space-y-2">
                {categoryBudgets.map((item, index) => {
                  ...
                  const opacity = Math.max(25, 100 - index * 14);
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <div className="mt-2 h-2 overflow-hidden rounded-full bg-secondary">
                            <div
                              ...
                              style={{
                                width: `${percentage}%`,
                                background: `color-mix(in oklab, var(--color-primary) ${opacity}%, transparent)`,
                              }}
                            />
                          </div>
                        </div>
          ...
          <div className="mt-6">
            <div className="mb-4">
              <h2 className="text-sm font-semibold">Spending type</h2>
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div className="rounded-2xl border border-border bg-card p-5">
                ...
                <p className="mt-2 text-xs text-muted-foreground">
                  {totalSpending > 0
                    ? Math.round((essentialSpending / totalSpending) * 100)
                    : 0}
                  % of total spending
                </p>
              </div>
              ...
              <div className="rounded-2xl border border-border bg-card p-5">
                ...
                <p className="mt-2 text-xs text-muted-foreground">
                  {totalSpending > 0
                    ? Math.round((discretionarySpending / totalSpending) * 100)
                    : 0}
                  % of total spending
                </p>
              </div>
                ...
                {categoryBudgets.map((item) => {
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-[11px] text-muted-foreground sm:hidden">
                            ...
                            · {transactionCounts[item.category] || 0}{" "}
                            transaction
                            {transactionCounts[item.category] === 1 ? "" : "s"}
                          </p>
                        </div>
                      </div>
                      ...
                      <span className="hidden text-xs text-muted-foreground sm:block">
                        {transactionCounts[item.category] || 0} transaction
                        {transactionCounts[item.category] === 1 ? "" : "s"}
                      </span>

                      <span className="text-sm font-semibold tabular-nums sm:text-right">
                        {formatMoney(item.last30DaySpend)}
                      </span>
                    </div>
                  );
                })}
          ...
          <div className="mt-6 rounded-2xl border border-border bg-card p-5">
            <div className="flex gap-3">
              ...
              <div>
                <h3 className="text-sm font-medium">How categories work</h3>

                <p className="mt-1 text-xs leading-relaxed text-muted-foreground">
                  Finwise automatically categorizes transactions when your
                  statement is uploaded. If a category is incorrect, you can
                  change it from the Transactions page. Your dashboard and
                  category analysis will then use the updated category.
                </p>
              </div>
            </div>
          </div>
